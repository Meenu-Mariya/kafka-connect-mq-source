...
public class SSLContextBuilderTest {
    ...
    @Test
    public void testBuildSslContextPrioritizesContentOverLocation() {
        final SSLContextBuilder builder = new SSLContextBuilder();
        
        // When both content and location are provided, content should take precedence
        final String validBase64Content = createTestKeystoreBase64();
        final Password keystoreContent = new Password(validBase64Content);
        final Password keystorePassword = new Password("changeit");
        final String keystoreLocation = "/non/existent/path/keystore.jks";
        
        try {
            final SSLContext sslContext = builder.buildSslContext(
                keystoreLocation, keystorePassword, keystoreContent,
                null, null, null
            );
            // If content is prioritized, it should attempt to use content first
            // and not fail on non-existent file location
            assertNotNull("SSLContext creation should prioritize content over location", sslContext);
        } catch (final ConnectException e) {
            // Expected if test keystore is invalid, but should not be file not found error
            if (e.getMessage().contains("non/existent/path")) {
                fail("Should prioritize content over location, not try to read from file");
            }
        }
    }
